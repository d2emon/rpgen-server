...
const data: { [key: string]: DataItem[] } = Object.keys(textData).reduce(
    (result, key) => ({
        ...result,
        [key]: textData[key].map((text, id): DataItem => ({
            id,
            text,
        })),
    }),
...
type ItemGenerator = () => DataItem;

export const generate = (): string => {
    const itemGenerators: { [key: string]: ItemGenerator} = {
        rnd1: () => generateItem(data.nm1),
        rnd2: () => generateItem(data.nm2),
        rnd3: () => generateItem(data.nm3),
        rnd4: () => generateItem(data.nm4),
        rnd5: () => generateItem(data.nm5),
        rnd6: () => generateItem(data.nm6),
        rnd7: () => generateItem(data.nm7),
        rnd8: () => generateItem(data.nm8),
        rnd9: () => generateItem(data.nm9),
        rnd10: () => generateItem(data.nm10),

        rnd11: () => generateItem(data.nm11),
        rnd12: () => generateItem(data.nm12),
        rnd13: () => generateItem(data.nm13),
        rnd14: () => generateItem(data.nm14),
        rnd15: () => generateItem(data.nm15),
        rnd16: () => generateItem(data.nm16),
        rnd17: () => generateItem(data.nm17),
        rnd17b: () => generateItem(data.nm17),
        rnd17c: () => generateItem(data.nm17),
        rnd18: () => generateItem(data.nm18),
        rnd19: () => generateItem(data.nm19),
        rnd20: () => generateItem(data.nm20),
        rnd20b: () => generateItem(data.nm20),
        rnd20c: () => generateItem(data.nm20),

        rnd21: () => generateItem(data.nm21),
    };
    ...
    if (items.rnd4.id < 15) {
        while(items.rnd5.id > 19){
            items.rnd5 = itemGenerators.rnd5();
        }
    }

    if (items.rnd7.id < 5) {
        while(items.rnd8.id > 4){
            items.rnd8 = itemGenerators.rnd8();
        }
    } else if (items.rnd7.id > 4 && items.rnd7.id < 10) {
        while(items.rnd8.id < 5 || items.rnd8.id > 9){
            items.rnd8 = itemGenerators.rnd8();
        }
    } else if (items.rnd7.id > 9 && items.rnd7.id < 15) {
        while(items.rnd8.id < 10 || items.rnd8.id > 14){
            items.rnd8 = itemGenerators.rnd8();
        }
    } else if (items.rnd7.id > 14) {
        while(items.rnd8.id < 15){
            items.rnd8 = itemGenerators.rnd8();
        }
    }

    while (items.rnd17b.id === items.rnd17.id) {
        items.rnd17b = itemGenerators.rnd17();
    }
    while ((items.rnd17b.id === items.rnd17c.id) || (items.rnd17c.id === items.rnd17.id)) {
        items.rnd17c = itemGenerators.rnd17();
    }

    while (items.rnd20b.id === items.rnd20.id) {
        items.rnd20b = itemGenerators.rnd20();
    }
    while ((items.rnd20b.id === items.rnd20c.id) || (items.rnd20c.id === items.rnd20.id)) {
